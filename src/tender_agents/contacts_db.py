...
class ContactRepository:
    ...
    async def upsert_contacts_batch(
        ...
        """Вставить / обновить контакты и их упоминания из импорта."""
        n = 0
        now = datetime.now(timezone.utc)
        # Группируем появления по индексу профиля в списке
        apps_by_idx: dict[int, list[ContactAppearance]] = {}
        for app in appearances:
            # Мы ожидаем, что app.profile_id может быть временным индексом в списке profiles
        ...
        async with self._session_factory() as session:
            for i, cp in enumerate(profiles):
                ...
                if prof:
                    ...
                    fs = _as_utc(prof.first_seen_at)
                    if cp.first_seen_at:
                        prof.first_seen_at = min(fs, _as_utc(cp.first_seen_at)) if fs else cp.first_seen_at
